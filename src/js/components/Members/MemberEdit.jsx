...
import { updateMember } from '../../client-api'
...
class MemberEdit extends React.Component {
  ...
  handleSubmit (e) {
    ...
    const {member} = this.props
      updateMember(this.state)
        .then(() => {this.props.fetchMembers()
          .then(() => this.props.history.push(`/members/member/${member.id}`))
          })
  ...
  render () {
    return (
      <form className='editMember' onSubmit={this.handleSubmit}>
        <h2 className=''>Edit Member</h2>
        <fieldset>
          <div className=''>
